...
var chat = {
	...
	doLogin : function( name , password ){
		...
		if(name == "" || password == ""){
			chat.displayError('chatErrorMessage_logout',"请输入昵称和Password才可以参与群聊哦～",1);
			return false;
		}
		//登录操作
		chat.data.type = 1; //登录标志
		chat.data.password = password; //邮箱
		chat.data.login = true;
		var json = {"type": chat.data.type,"name": name,"password": password,'roomid':'a'};
		$.ajax({
            type        : 'post',
            url         : 'chat_login',
            ...
            success     : function(e){
                if(e.code  == 200){
                    chat.displayError('newlogin','登陆成功',0);
                    setTimeout("window.location.reload()",2000);
                }else{
                    chat.displayError('chatErrorMessage_logout',e.msg,0);
                }
            },
            error    : function(e) {
                chat.displayError('chatErrorMessage_logout',"登陆出错了",1);
           }
        });
	...
	wsOpen : function (){
		this.data.wSock.onopen = function( event ){
			...
			if( isLogin ) {
				...
				var json = {"type": 1,"name": name,"password": 123456,'roomid':'a'};
				chat.wsSend(JSON.stringify(json));
				chat.data.login = true;
			}
	...
	changeRoom : function(obj){
		...
		var json = {"type": chat.data.type,"name": chat.data.name,"avatar": chat.data.avatar,"oldroomid":oldroomid,"roomid":this.data.crd,"password":chat.data.storage.getItem("password")};
		chat.wsSend(JSON.stringify(json));
